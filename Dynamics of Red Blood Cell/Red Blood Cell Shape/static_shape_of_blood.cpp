...
int main() {
    ...
    int latitudeDivisions = 512;      // Higher resolution
    int longitudeDivisions = 512;     // Higher resolution
    float deformationFactor = 0.8f;   // Adjustable concave deformation factor
    float heightFactor = 0.8f;        // Adjustable height factor
    int totalFrames = 1;            // Number of frames for animation
    float timeStep = 1.0f;            // Time increment per frame
    int n = 0;                        // Energy level n (number of oscillations in theta direction)
    int m = 0;                        // Energy level m (number of oscillations in phi direction)
    std::string outputFile = "red_blood_cell_shape_data.txt";
